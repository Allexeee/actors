  ...
  sealed class ProcessorGroups
  {
    public static void Setup(object b)
    {
      ...
      for (int i = 0; i < length; i++)
      {
        ...
        if (myFieldInfo.FieldType.IsSubclassOf(groupType))
        {
          ...
          var composition = new Composition(HashCode.OfEach(myFieldInfo.FieldType.GetGenericArguments()).And(31));
          ...
          else if (groupRemovedAttribute != null)
          {
    ...
    internal static GroupCore SetupGroup(Type groupType, Composition composition, object fieldObj)
    {
      ...
      if (fieldObj != null)
      {
        ...
        return groups.globals[groups.globalsLen++] = groups.All.Add(CreateGroup(groups.All, groupType, composition));
      }
